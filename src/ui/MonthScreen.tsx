...
import type { ActionId, LogEntry, TrainingState } from '../engine/types';
  ...
  switch (phase) {
    ...
    case 'Live Period':
      return {
        ...
        sub: 'The month coaches are allowed to watch. Offers land or die here.',
        gamesFirst: false,
      };
    default:
      return {
...
export default function MonthScreen({
  ...
  // Keyed off the games being shown, not the current phase: in March the panel
  // is still displaying February's regular season slate.
  const games = (
    <GamesPanel
  ...
  return (
    <div className="mx-auto max-w-5xl px-8 py-10">
      ...
      <div className="mt-6 text-sm text-neutral-400">
        ...
        {season && (
          <span className="text-neutral-600">
            {' '}
            · {season.gradeLabel} · {season.wins}–{season.losses}
          </span>
        )}
      </div>

      <dl className="mt-4 grid grid-cols-6 gap-px overflow-hidden rounded-lg border border-neutral-800 bg-neutral-800">
        ...
        <Stat label="Energy" value={String(view.energy)} tone={view.energy < 35 ? 'bad' : undefined} />
        <Stat label="Coach trust" value={String(view.coachTrust)} />
        <Stat
          label="Proj. min"
          value={view.injury ? '—' : view.projectedMinutes.toFixed(0)}
        />
      </dl>
      ...
      {view.injury && (
        <div className="mt-4 rounded-lg border border-red-900/60 bg-red-950/30 px-5 py-3">
          ...
          <div className="mt-0.5 text-sm text-red-400/80">
            ...
            {view.injury.monthsRemaining === 1 ? '' : 's'} of rehab left. You are
            not playing until it heals.
          </div>
        </div>
      )}
      ...
      <div className="mt-8 grid grid-cols-[1fr_18rem] gap-10">
        <div className="space-y-9">
          ...
          <button
            ...
            onClick={onNextMonth}
            disabled={saving}
            className="rounded-md bg-orange-600 px-6 py-3 text-lg font-medium hover:bg-orange-500 disabled:cursor-not-allowed disabled:bg-neutral-800 disabled:text-neutral-600"
          >
